...
# The IP/domain without a scheme
API_BASE = 'localhost:8050'
...
async def browse_session(browser_num, start_delay):
    ...
    try:
        ...
        resp = api_request.post(f"{API_URL}/browsers", timeout=30)
        ...
        for retry in range(max_retries):
            try:
                details = api_request.get(f"{API_URL}/browsers/{browser_id}", timeout=30).json()
        ...
        if websocket_url.startswith("ws"):
            ...
            ws_url = f"ws://{API_BASE}{websocket_url}"
        ...
        if browser_id:
            try:
                ...
                api_request.delete(f"{API_URL}/browsers/{browser_id}", timeout=10)
            except Exception as e:
